...
use crate::structs::keys::{
        chain_key::ChainKey, message_key::MessageKey,
...
impl RatchetState {
    ...
    pub(crate) fn new(
        ...
        let (sending_chain, receiving_chain) = if is_initiator {
            crate::crypt::crypto_utils::hkdf::derive_initial_chain_keys(&root_key)
        } else {
            let (recv, send) = crate::crypt::crypto_utils::hkdf::derive_initial_chain_keys(&root_key);
            (send, recv)
        };
        Self {
            root_key,
    ...
    ///
    /// # Returns
    /// An `Message` containing ciphertext and metadata.
    ...
    /// Attempts to decrypt a received `Message`.
    ///
    ...
    pub(crate) fn decrypt(&mut self, msg: &Message) -> Option<String> {
